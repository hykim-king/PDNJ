...
public class LoginController {
	...
	public String doLogin(UserVO user, HttpSession httpSession) throws SQLException{
		...
		if(null == user.getUserId() || "".equals(user.getUserId())) {
			message.setMsgId("1");
			message.setMsgContents("아이디를 입력 하시오.");
		...
		if(null == user.getPassword() || "".equals(user.getPassword())) {
			message.setMsgId("2");
			message.setMsgContents("비밀번호를 입력 하시오.");
		...
		if(10 == check) {//id확인
			message.setMsgId("10");
			message.setMsgContents("아이디를 확인 하시오.");
			...
			message.setMsgId("30");
			message.setMsgContents(outVO.getUsername()+"님 반갑습니다.");
